...
namespace HRMS.Pages
{
    public partial class Dashboard
    {
        ...
        public int _TodayleaveStatus = 0;
        HRMS.Model.Dashboard _DashboardEvent = new HRMS.Model.Dashboard();
        HRMS.Model.Dashboard _DashboardEvent1 = new HRMS.Model.Dashboard();
        List<HRMS.Model.Dashboard> _DashboardList = new List<HRMS.Model.Dashboard>();
        List<HRMS.Model.Dashboard> _DashboardList1 = new List<HRMS.Model.Dashboard>();
        ...
        public void OpenInLineDialog(int EventId)
        {
            ...
            var list = _DashboardList.FirstOrDefault(x => x.EventId == EventId);
            if (list != null)
            {
                _DashboardEvent.EventId = list.EventId;
                _DashboardEvent.EventName = list.EventName;
                _DashboardEvent.TimeIn = list.TimeIn;
                _DashboardEvent.TimeOut = list.TimeOut;
            }
        ...
        public async Task Submit()
        {
            try
            {
                ...
                string apiUrl = $"api/TimeSchedule/TimeUpdate/{_DashboardEvent.EventId}";
                using var client = HttpClientFactory.CreateClient("API");
                var response = await client.PutAsJsonAsync(apiUrl, _DashboardEvent).ConfigureAwait(false);
                if (response.IsSuccessStatusCode)
                {
                    await TimeSchedule();
        ...
        private void AddTimeDialog() => visible1 = true;
        ...
        public async Task Saves()
        {
            try
            {
                ...
                const string apiUrl = "api/TimeSchedule";
                using var client = HttpClientFactory.CreateClient("API");
                var response = await client.PostAsJsonAsync(apiUrl, _DashboardEvent).ConfigureAwait(false);
                if (response.IsSuccessStatusCode)
                {
                    await TimeSchedule();
        ...
        {
            visible = false;
            string apiUrl = $"api/TimeSchedule/{_DashboardEvent.EventId}";
            using var client = HttpClientFactory.CreateClient("API");
            var response = await client.PutAsJsonAsync(apiUrl, _DashboardEvent).ConfigureAwait(false);
            if (response.IsSuccessStatusCode)
            {
                await TimeSchedule();
                Snackbar.Add("Event Dekete Successfully", Severity.Success, config =>
                {
                    config.VisibleStateDuration = 200;
                });
            }
