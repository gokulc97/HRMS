...
namespace HRMS.Pages
{
	public partial class ChangePassword
	{
		...
		{
			var userpass = LoginEvent.FirstOrDefault(x => x.Password == _login.Password);
			//await LoginDetails();
			if (userpass != null)
			{
				if (_login.Password == userpass.Password && userpass.Password != _login.NewPassword && pass is not null)
				{
				...
				{
					error = "Old Password And New Password Should Not Be Same";
				}
			}
		...
		public async Task LoginDetails()
		{
			try
			{ 
			string apiUrl = $"api/LoginDetails?user={User.UserName}&password={_login.Password}";
